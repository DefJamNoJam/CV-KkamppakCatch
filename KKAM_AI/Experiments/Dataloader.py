...
class VideoDataset(Dataset):
    def __init__(self, args, root_dir, transform=None):
        ...
        self.args = args
        self.transform = transform
        self.classes = ['Normal', '전도', '파손', '절도']  # 필요한 클래스만
        ...
        for cls_name in self.classes:
            ...
            for x_path in sorted(glob.glob(os.path.join(x_dir, '*.npy'))):
                ...
                self.samples.append((x_path, label))

        if not self.samples:
            raise RuntimeError(f'No X.npy found in {root_dir}')

    def __len__(self):
        return len(self.samples)

    def __getitem__(self, idx):
        x_path, label = self.samples[idx]
        x_data = np.load(x_path)  # (T, H, W, C) or (T, C, H, W)
        if x_data.shape[-1] == 3:
            x_data = np.transpose(x_data, (0, 3, 1, 2))

        seq_len = self.args.window_size * self.args.fps
